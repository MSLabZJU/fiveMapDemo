...
public class GyroscopeOrientation extends Orientation
{
	...
	private static final String tag = GyroscopeOrientation.class.getSimpleName();
	...
	private void getRotationVectorFromGyro()
	{
		...
		vGyroscope[2] += 0.004;
		//Log.d(tag, "vGyroscope[2]:" + vGyroscope[2]);

		//use threshold
		if (Math.abs(vGyroscope[2]) < threshold)
			return;
		...
		qGyroscope = qGyroscope.multiply(qGyroscopeDelta);	//鏇存柊鍥涘厓鏁皅Gyroscope銆�
	}
	...
	protected void onGyroscopeChanged()
	{
		...
		{
			//dT:time between two vGyroscope.
